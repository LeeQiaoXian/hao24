! function(N, M) {
	...
	function G(b, l) {
		...
		l = l || I(b);
		for (var k = l.frag.cloneNode(), j = 0, f = K(), d = f.length; d > j; j++) {
			k.createElement(f[j])
		}
		return k
	}
	...
	function F(d, c) {
		c.cache || (c.cache = {}, c.createElem = d.createElement, c.createFrag = d.createDocumentFragment, c.frag = c.createFrag()), d.createElement = function(a) {
			return u.shivMethods ? H(a, d, c) : c.createElem(a)
		}, d.createDocumentFragment = Function("h,f", "return function(){var n=f.cloneNode(),c=n.createElement;h.shivMethods&&(" + K().join().replace(/[\w\-:]+/g, function(b) {
	...
	function E(b) {
		b || (b = M);
		var c = I(b);
		return !u.shivCSS || D || c.hasCSS || (c.hasCSS = !!L(b, "article,aside,dialog,figcaption,figure,footer,header,hgroup,main,nav,section{display:block}mark{background:#FF0;color:#000}template{display:none}")), C || F(b, c), b
	}
	var D, C, B = "3.7.2",
		A = N.html5 || {},
	...
	! function() {
		try {
			...
			b.innerHTML = "<xyz></xyz>", D = "hidden" in b, C = 1 == b.childNodes.length || function() {
				M.createElement("a");
				var c = M.createDocumentFragment();
